<script lang="ts">
  ...
  /** Track which cell is being edited: "rowIndex:column" */
  ...
  /** Count of error rows (excluding skipped rows) */
    ...
    <table class="min-w-full divide-y divide-gray-200">
      <thead class="sticky top-0 bg-gray-50">
        <tr>
          <th
            scope="col"
            class="w-10 px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500"
          >
            #
          </th>
          ...
          <th
            scope="col"
            class="w-16 px-3 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500"
          >
            Skip
          </th>
        </tr>
            ...
            {#each columns as col (col.key)}
              <td
                class="whitespace-nowrap px-3 py-2 text-sm
                  {isSkipped ? 'text-gray-400 line-through' : 'text-gray-900'}"
              >
                {#if col.editable && editingCell === `${rowIndex}:${col.key}` && !isSkipped}
                  <input
                    type="text"
                    class="w-20 rounded border border-blue-400 px-1 py-0.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                ...
                {:else if col.editable && !isSkipped}
                  <button
                    type="button"
                    class="cursor-text rounded px-1 py-0.5 text-left hover:bg-blue-50 hover:ring-1 hover:ring-blue-300"
