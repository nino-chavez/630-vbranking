<script lang="ts">
	...
	function handleMapTo(
		...
		suggestion: { id: string; name: string; code?: string },
	) {
		const mapping: IdentityMapping = {
			type: conflict.type,
			parsedValue: conflict.parsedValue,
	...
	function getFilteredSuggestions(conflict: IdentityConflict) {
		...
		return conflict.suggestions.filter(
			(s) =>
				s.name.toLowerCase().includes(lower) || (s.code && s.code.toLowerCase().includes(lower)),
		);
	}
									...
									>
										<div class="border-b border-border p-2">
											<input
												...
												oninput={(e) =>
													updateSearchFilter(conflict, (e.target as HTMLInputElement).value)}
											/>
										</div>
