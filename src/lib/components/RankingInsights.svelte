<script lang="ts">
	import type { RankingInsight } from '$lib/ranking/stats.js';

	let { insights }: { insights: RankingInsight[] } = $props();
</script>

<div class="flex flex-wrap gap-2">
	{#each insights as insight (insight.id)}
		{@const isWarning = insight.severity === 'warning'}
		{@const isClean = insight.id === 'clean'}
		<span
			class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium {isWarning
				? 'bg-amber-50 text-amber-800 dark:bg-amber-950 dark:text-amber-300'
				: isClean
					? 'bg-emerald-50 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300'
					: 'bg-blue-50 text-blue-800 dark:bg-blue-950 dark:text-blue-300'}"
			title={insight.detail}
		>
			{#if isWarning}
				<svg class="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="currentColor">
					<path
						fill-rule="evenodd"
						d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.345 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 6a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 6zm0 9a1 1 0 100-2 1 1 0 000 2z"
						clip-rule="evenodd"
					/>
				</svg>
			{:else if isClean}
				<svg class="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="currentColor">
					<path
						fill-rule="evenodd"
						d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z"
						clip-rule="evenodd"
					/>
				</svg>
			{:else}
				<svg class="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="currentColor">
					<path
						fill-rule="evenodd"
						d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z"
						clip-rule="evenodd"
					/>
				</svg>
			{/if}
			{insight.label}
		</span>
	{/each}
</div>
