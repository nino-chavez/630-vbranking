...
import type { IdentityConflict, IdentityMapping, ParseError } from '$lib/import/types.js';
...
describe('IdentityResolutionPanel conflict resolution logic', () => {
	...
	it('produces one mapping per conflict when Skip is clicked, and calls onResolve with correct mapping', () => {
		const conflicts: IdentityConflict[] = [
			{
				type: 'team',
				parsedValue: 'ACEVB',
				suggestions: [{ id: 'team-1', name: 'Ace VB', code: 'ACE', score: 0.8 }],
			},
			{
				type: 'tournament',
		...
		const tournamentMapping = result.mappings.find((m) => m.type === 'tournament');
	...
	it('tracks unresolved count correctly when only some conflicts are resolved', () => {
		...
		const result = resolveConflicts(conflicts, [{ conflict: conflicts[0], action: 'skip' }]);
...
describe('DataPreviewTable error and skip logic', () => {
	...
	function getRowErrors(errors: ParseError[], rowIndex: number): ParseError[] {
...
describe('Import page state machine derived state', () => {
	...
	function computeCanConfirm(
		...
		const allResolved =
			...
			conflicts.every((conflict) =>
				mappings.some((m) => m.type === conflict.type && m.parsedValue === conflict.parsedValue),
			);
	...
	it('returns false when conflicts are unresolved', () => {
		const conflicts: IdentityConflict[] = [{ type: 'team', parsedValue: 'ACEVB', suggestions: [] }];
	...
	it('returns true when all conflicts resolved and error rows are skipped', () => {
		const conflicts: IdentityConflict[] = [{ type: 'team', parsedValue: 'ACEVB', suggestions: [] }];
		const mappings: IdentityMapping[] = [{ type: 'team', parsedValue: 'ACEVB', action: 'skip' }];
		const errors: ParseError[] = [
