...
function buildColleyFixture(): ArrayBuffer {
  ...
  const data: (string | number)[][] = [
    ['Alpha',   'ALP', 20, 5, 0.85, 1, 0.82, 2, 0.90, 1, 0.88, 1, 0.79, 3, 0.848, 1],
    ['Bravo',   'BRV', 18, 7, 0.78, 2, 0.84, 1, 0.75, 3, 0.80, 2, 0.82, 1, 0.798, 2],
    ['Charlie', 'CHL', 15, 10, 0.65, 3, 0.70, 3, 0.68, 4, 0.72, 3, 0.71, 4, 0.692, 3],
    ['Delta',   'DLT', 12, 13, 0.55, 4, 0.58, 4, 0.80, 2, 0.60, 4, 0.62, 5, 0.630, 4],
    ['Echo',    'ECH', 8,  17, 0.40, 5, 0.42, 5, 0.45, 5, 0.38, 5, 0.80, 2, 0.490, 5],
  ];
...
function buildColleyFixtureWithErrors(): ArrayBuffer {
  ...
  const data: (string | number)[][] = [
    ['Alpha',   'ALP', 20, 5, 0.85, 1, 0.82, 2, 0.90, 1, 0.88, 1, 0.79, 3, 0.848, 1],
    ['BadTeam', 'BAD', 10, 3, 'N/A' as unknown as number, 'abc' as unknown as number, 0.82, 2, 0.90, 1, 0.88, 1, 0.79, 3, 0.848, 1],
  ];
...
describe('ColleyParser', () => {
  ...
  it('correctly maps all 16 columns to ParsedColleyRow fields', () => {
    ...
    expect(alpha.algo3Rating).toBeCloseTo(0.90);
    ...
    expect(echo.algo5Rating).toBeCloseTo(0.80);
    expect(echo.algo5Rank).toBe(2);
    expect(echo.aggRating).toBeCloseTo(0.490);
