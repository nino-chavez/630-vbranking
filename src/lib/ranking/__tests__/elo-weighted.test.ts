...
describe('Weighted Elo', () => {
  it('scales K-factor by tournament weight -- double weight produces double rating change', () => {
    ...
    const weighted = computeEloRatings(singleGame, teams, startRating, DEFAULT_K_FACTOR, { T1: 2.0 });
  ...
  it('produces identical results when no weight map is provided', () => {
    ...
    const withUndefined = computeEloRatings(singleGame, teams, startRating, DEFAULT_K_FACTOR, undefined);
