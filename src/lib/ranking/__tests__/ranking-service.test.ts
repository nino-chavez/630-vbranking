		...
		from: vi.fn((table: string) => {
			...
			if (table === 'tournament_weights') {
				return {
					select: () => ({
						eq: () => ({
							in: () => Promise.resolve(overrides.tournament_weights ?? { data: [], error: null }),
						}),
					}),
				};
			}
				...
				return {
					...
					delete: () => ({
						eq: () => {
							...
							return Promise.resolve(overrides.ranking_runs_delete ?? { error: null });
						},
					}),
				};
			}
			...
			if (table === 'ranking_results') {
				return {
					insert: () => Promise.resolve(overrides.ranking_results_insert ?? { error: null }),
					select: () => ({
						eq: () => ({
							order: () =>
								Promise.resolve(overrides.ranking_results_select ?? { data: [], error: null }),
						}),
					}),
					delete: () => ({
						eq: () => {
							deleteTracker.ranking_results_deleted = true;
							return Promise.resolve(overrides.ranking_results_delete ?? { error: null });
						},
					}),
				};
			}
...
describe('RankingService', () => {
	...
	it('throws for invalid age_group', async () => {
		...
		await expect(service.runRanking({ ...defaultConfig, age_group: 'INVALID' })).rejects.toThrow(
			'Invalid age_group',
		);
	});
