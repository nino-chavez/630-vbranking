	...
	async function handleSave() {
		...
		try {
			const changedWeights = weights
				.filter(
					(w, i) => w.weight !== originalWeights[i].weight || w.tier !== originalWeights[i].tier,
				)
				.map((w) => ({
		...
		<Card>
			...
			<div class="overflow-x-auto">
				<table class="w-full text-sm">
					<thead>
						<tr
							class="border-b border-border text-left text-xs font-semibold uppercase tracking-wider text-text-muted"
						>
					...
					<tbody class="divide-y divide-border">
						{#each weights as row, i (row.tournament_id)}
							<tr class="hover:bg-surface-alt/50">
								<td class="whitespace-nowrap px-3 py-2 font-medium text-text-primary"
									>{row.tournament_name}</td
								>
								<td class="whitespace-nowrap px-3 py-2 text-text-secondary"
									>{row.tournament_date}</td
								>
								<td class="px-3 py-2">
									<select
								...
								<td class="px-3 py-2">
									...
									{:else}
										<span
											class="inline-flex items-center rounded-full bg-surface-alt px-2 py-0.5 text-xs font-medium text-text-muted"
											>Default</span
										>
									{/if}
								</td>
