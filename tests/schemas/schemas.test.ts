...
describe('matchSchema', () => {
	...
	it('rejects invalid winner_id (not a participant)', () => {
		...
		if (!result.success) {
			const messages = result.error.issues.map((i) => i.message);
			expect(messages).toContain('winner_id must be either team_a_id or team_b_id when not null');
		}
	});
...
describe('tournamentResultSchema', () => {
	...
	it('rejects finish_position > field_size', () => {
		...
		if (!result.success) {
			const messages = result.error.issues.map((i) => i.message);
			expect(messages).toContain('finish_position must be less than or equal to field_size');
		}
	});
});
